  ...
  Widget build(BuildContext context) {
    return Scaffold(
                      ...
                      onPressed: () {
                        if (formKey.currentState!.validate()) {
                          if (int.parse(columnController.text) > 7) {
                            ...
                            homeScreenController.rows.value = int.parse(
                                rowsController.text.isEmpty
                                    ? "0"
                                    : rowsController.text);
                            homeScreenController.column.value = int.parse(
      ...
      body:  Padding(
        ...
        child: Obx(
              () => homeScreenController.generatedChars.isNotEmpty &&
                (homeScreenController.rows.value > 0)
            ? GridView.builder(
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                    crossAxisCount: homeScreenController.column.value),
                itemBuilder: (context, index) {
                  // return Text("$rows / $column");
                  return Obx(
                    () => Container(
                      width: (screenWidth(context) ~/
                              homeScreenController.column.value)
                          .toDouble(),
                      height: (screenHeight(context) ~/
                              homeScreenController.rows.value)
                          .toDouble(),
                      margin:
                      ...
                      child: FittedBox(
                        child: Padding(
                          ...
                          child: Center(
                            child: Text(
                              homeScreenController.generatedChars[index],
